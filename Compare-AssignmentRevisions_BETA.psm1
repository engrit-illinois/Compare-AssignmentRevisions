...
function Compare-AssignmentRevisions {
	...
	function Get-CompData($comps) {
		...
		foreach($comp in $comps) {
			...
			# Just setting to null didn't prevent memory from accumulating
			# Tried the below per https://stackoverflow.com/questions/63306163/how-can-i-diagnose-which-of-my-powershell-variables-resources-are-eating-memory
			# But these (at this point in the script) also did nothing.
			#[System.GC]::Collect()
	...
	function Do-Stuff {
		...
		if($compNames) {
			...
			$comps = $null
			$script:CachedAppDeployments = $null
			$script:CachedTSDeployments = $null
			$script:CachedApplications = $null
			$script:CachedTaskSequences = $null
			$script:CachedCollections = $null
			[System.GC]::Collect()
			[System.GC]::WaitForPendingFinalizers()
		}
